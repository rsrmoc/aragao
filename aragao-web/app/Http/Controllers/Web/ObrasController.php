...
class ObrasController extends Controller {
    public function renderizarRelatorio($idRelatorio) {
        ...
        $etapas = ObrasEtapas::where('id_obra', $obra->id)->orderBy('created_at', 'desc')->get();
        $porcGeral = ObrasEtapas::where('id_obra', $obra->id)->where('created_at', '<=', $relatorio->created_at)->get()->sum('insidencia_executada');
        $evolucoes = ObrasEvolucoes::with(['etapa', 'usuario'])->where('id_obra', $obra->id)->where('created_at', '<=', $relatorio->created_at)->orderBy('created_at', 'desc')->get();
