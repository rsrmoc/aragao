...
class EtapasTabAditivos extends Component
{
    use WithPagination;
    use WithFileUploads;
    ...
    function saveAditivo() {
        if ($this->editId) return $this->editAditivo();
    ...
    function addAditivo() {
        ...
        try {
            ...
            $aditivo = ObrasAditivos::create($data);

            if (count($this->inputsImages) >= 1) {
                foreach ($this->inputsImages as $image) {
                    $dados = [
                        'tabela_id' => $aditivo->id,
                        'tabela_type' => 'App\Models\ObrasAditivos',
            ...
            $this->dispatch('toast-event', 'Aditivo adicionado!', 'success');
        }
        catch(Exception $e) {
            DB::rollBack();
    ...
    function editAditivo() {
        ...
        try {
            ...
            $aditivo = ObrasAditivos::find($this->editId);
            $aditivo->update($data);
            ...
            $this->resetExcept('obra');
            $this->dispatch('clear-file-input');
            $this->dispatch('toast-event', 'Aditivo atualizado!', 'success');
        }
        catch(Exception $e) {
            DB::rollBack();
    ...
    public function excluirAditivo(int $idAditivo) {
        try {
            ObrasAditivos::find($idAditivo)->delete();

            $this->dispatch('toast-event', 'Aditivo excluido!', 'success');
        }
        catch(Exception $e) {
            $this->dispatch('toast-event', $e->getMessage(), 'error');
        }
    ...
    public function render()
    {
        ...
        $aditivos = ObrasAditivos::with(['imagens', 'usuario'])->where('id_obra', $this->obra)->orderBy('created_at', 'desc')->paginate(12);

        return view('livewire.components.views.obras.etapas-tab-aditivos', compact('aditivos', 'etapas'));
    }
}
